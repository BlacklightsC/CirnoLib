...
namespace CirnoLib.Format.BLPLib.Legacy
{
	...
	public abstract class Surface : IDisposable, ICloneable
	{
		...
		public Surface(int width, int height, byte alphaBitCount, bool alphaPremultiplied = false)
		{
			...
			if (sizeof(int) * (long)width * height > int.MaxValue) throw new ArgumentException();
			...
			this.IsAlphaPremultiplied = alphaPremultiplied;
		}
		...
		protected Surface(Surface surface)
		{
			...
			this.IsAlphaPremultiplied = surface.IsAlphaPremultiplied;
		}
        ...
        public int AlphaBitCount { get => alphaBitCount; }
        ...
        public bool IsAlphaPremultiplied { get; }
		...
		public SurfaceData Lock()
		{
            ...
            var dataPointer = LockInternal(out int stride);
		...
		public abstract class Wrapper : Surface
		{
            public Wrapper(Surface surface)
				: base(surface) => this.BaseSurface = surface;

			protected override void Dispose(bool disposing)
			{
				if (disposing && BaseSurface != null)
				{
					BaseSurface.Dispose();
					BaseSurface = null;
				}
            ...
            public Surface BaseSurface { get; private set; }

            public sealed override bool CanLock { get => BaseSurface.CanLock; }

			protected sealed override IntPtr LockInternal(out int stride) => BaseSurface.LockInternal(out stride);

			protected sealed override void UnlockInternal() => BaseSurface.UnlockInternal();

			protected sealed override void CopyToArgbInternal(SurfaceData surfaceData) => BaseSurface.CopyToArgbInternal(surfaceData);

			public sealed override byte[] ToArray() => BaseSurface.ToArray();

			public sealed override Stream CreateStream() => BaseSurface.CreateStream();
		}
	}
}
